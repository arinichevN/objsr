...
int acpspcParam_set(ACPSPortControllerParam *self, const char *serial_filename, int serial_rate, const char *serial_config){
	self->filename = NULL;
...
void acpspcParam_free(ACPSPortControllerParam *self){
	free(self->filename);
}
